...
void markerArrayCallback(const visualization_msgs::MarkerArray::ConstPtr& msg){
    for (int i = 0; i < msg->markers.size();  i++) {
            double x = msg->markers[i].scale.x;
            double y = msg->markers[i].scale.y;
            test_points.push_back(std::make_pair(x,y));
        std::cout << "markerArrayCallback: " << x << " " << y << std::endl;
    }
}
  ...
  for (int i = 0; i < num_of_points_; ++i) {
    ...
    {   
        ...
        vel_obs = std::sqrt(std::pow(corner_points_vel[j].first, 2) + std::pow(corner_points_vel[j].second, 2));
         if (distance_obs2refpoint<obs_threshold && vel_obs<0.01)
         {  
          is_on_road = true;
          break;
        }
